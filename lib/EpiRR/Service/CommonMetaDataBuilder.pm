  ...
  for my $s (@samples) {
    ...
    for my $k ( keys %meta_data ) {
      delete $meta_data{$k}
        ...
        || lc( $s->get_meta_data($k) ) ne lc( $meta_data{$k} ) );
    }
  }
...
sub check_sample_meta_data {
  ...
  for my $required_metadata ( $self->all_required_sample_meta_data() ) {
    ...
    if ( !$found ) {
      my $sid = $s->sample_id();
      push @$errors,
        "Meta data for sample $sid should include one of the following: "
...
sub check_common_meta_data {
  ...
  for my $required_metadata ( $self->all_required_common_meta_data() ) {
    ...
    if ( !$found ) {
      push @$errors, "Common meta data should include one of the following: "
        . join( ', ', @$required_metadata );
    }
  }
}
