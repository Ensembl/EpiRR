...
has 'required_experiment_types' => (
    ...
    isa     => 'ArrayRef[Str]',
    traits  => ['Array'],
    default => sub { ['DNA Methylation','mRNA-seq','ChIP-Seq Input',] },
    handles => {
      'all_required_experiment_types' => 'elements'
    }
);
...
sub determine_classification {
    ...
    my $completeness = $self->experimental_completeness($dataset_version,$errors);
...
sub composition {
  ...
  for my $s (@$samples){
    if ($s->get_meta_data('POOL_ID')){
      ...
      $donors{$s->get_meta_data('DONOR_ID') || $s->get_meta_data('LINE')}++;
    }
    else {
      push @$errors, 'No DONOR_ID/POOL_ID/LINE found for sample '.$s->sample_id();
    }
  }
