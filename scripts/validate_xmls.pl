...
my $outfile;
my $quiet = 0;
my $python_script_dir; 

GetOptions(
  "config=s" => \$config_module,
  "dir=s"    => \$dir,
  "python_script_dir=s"    => \$python_script_dir,
  'quiet!'   => \$quiet,
  ...
  if ( $raw_data->archive->name eq 'EGA' || $raw_data->archive->name eq 'ENA' || $raw_data->archive->name eq 'DDBJ' ) {
    ...
    if ( $raw_data->dataset_version->is_current ) {
      ...
      foreach my $n (@list_of_EGAX) {
        ...
        my $cmd = "python $python_script_dir/ihec-ecosystems/version_metadata/__main__.py -experiment -config:lib/ihec-ecosystems/version_metadata/config_edited.json -out:./".$exp_id.".versioned.xml ./".$exp_id.".xml";# 2> /dev/null";
        system($cmd);

        my $exp_val_flag="False"; my $exp_val_errors=""; my @exp_val_log_files=glob("errs.experiment*.log");
        if ( scalar @exp_val_log_files == 1) {
          print "Found error logs when validating: ".@exp_val_log_files."\n";
          ...
          $cmd = "python $python_script_dir/ihec-ecosystems/version_metadata/__main__.py -experiment -extract -config:lib/ihec-ecosystems/version_metadata/config_edited.json -out:./".$exp_id.".versioned.xml ./".$exp_id.".xml"; #2> /dev/null";/
          system($cmd);

          $cmd = "python $python_script_dir/ihec-ecosystems/version_metadata/review.py -sample ./".$exp_id.".xml.extracted.json";
          ...
          exit;
        } else { print "Experiment $exp_id validates!\n"; $exp_val_flag="True"; }
        ...
        $cmd = "python $python_script_dir/ihec-ecosystems/version_metadata/__main__.py -sample -config:lib/ihec-ecosystems/version_metadata/config_edited.json -out:./".$sample_id.".versioned.xml ./".$sample_id.".xml"; #2> /dev/null";
        system($cmd);
        ...
        if ( scalar @sample_val_log_files == 1) {
          ...
          $cmd = "python $python_script_dir/ihec-ecosystems/version_metadata/__main__.py -sample -extract -config:lib/ihec-ecosystems/version_metadata/config_edited.json -out:./".$sample_id.".versioned.xml ./".$sample_id.".xml"; #2> /dev/null";
          system($cmd);

          $cmd = "python $python_script_dir/ihec-ecosystems/version_metadata/review.py -sample ./".$sample_id.".xml.extracted.json";
          my $review_xml_output = `$cmd`;
          chomp $review_xml_output;
